...
async function getGlitch(id: string): Promise<Glitch | null> {
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}/api/glitches/${id}`, {
      cache: 'no-store',
    });
...
export default function GlitchDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
